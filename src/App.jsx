...
import { Users, Brain, RotateCcw, TrendingUp, Zap, Target, Cpu, Clock, Activity, Lightbulb, Play, Network, LogOut, Loader, Plus, List, AlertTriangle, Search } from 'lucide-react'; // Added Search icon
...
const ReversiGame = () => {
  ...
  // NEW: State for the manual join input field
  const [manualGameId, setManualGameId] = useState(''); 


  // --- Game Logic Wrappers (Memoized functions) ---
  ...
  useEffect(() => {
    ...
    socket.on('game_created', (data) => {
      ...
      setOnlinePlayerColor(data.player_color); // 1 (Black)
      setOnlineStatus('waiting');
      setBoardSize(data.board_size);
      setGameState('online_playing');
      initializeGame(data.board_size, 'online'); // Initial board setup
    });

    socket.on('game_joined', (data) => {
      setOnlineGameId(data.game_id);
      setOnlinePlayerColor(data.player_color); // 2 (White)
  ...
  // NEW: Effect to automatically fetch games when entering the lobby
  useEffect(() => {
  ...
  const renderCell = (row, col) => {
    ...
    return (
      ...
      >
        ...
        {isValid && isCurrentPlayerHumanControlled && (
  ...
  if (gameState === 'online_lobby') {
    const fetchOpenGames = () => socket.emit('get_open_games');
    
    const createGame = () => {
      if (socket) {
        socket.emit('create_game', { board_size: boardSize });
      }
    };
    const joinGame = (id) => {
      if (socket && id) {
      ...
      <div className="min-h-screen bg-gray-900 p-4">
        <div className="max-w-4xl mx-auto bg-gray-800 rounded-2xl shadow-2xl p-8 border-2 border-green-600">
          ...
          <div className="flex justify-between items-center p-4 bg-gray-700 rounded-lg mb-6 text-white">
            ...
            <p className="flex items-center gap-2">
              {isOnlineReady ? <span className="text-green-500">🟢 Connected</span> : <span className="text-red-500">🔴 Disconnected</span>}
            </p>
          </div>
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto bg-gray-900/50 p-4 rounded-xl border border-gray-700">
            {openGames.length === 0 ? (
              <p className="text-gray-400 text-center py-4">No open games found. Create one or refresh!</p>
            ) : (
              openGames.map((game) => (
  ...
  // 2. Online Playing Screen (Combined with Local Playing)
  if (gameState === 'online_playing') {
    // If the game is just created and waiting
        ...
        <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-lg w-full text-center border-2 border-yellow-600">
            ...
            <button
              ...
              className="mt-6 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg flex items-center mx-auto gap-2"
            >
      ...
      return (
        <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-lg w-full text-center border-2 border-red-600">
            ...
            <button
              onClick={resetOnlineState}
              className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center mx-auto gap-2"
            >
              Back to Menu
            </button>
          </div>
    ...
    // If the game is in progress, fall through to the main game screen below.
    // Set gameMode to online so the main game screen renders correctly.
    // NOTE: This side-effect inside render is risky, but safe if specific conditions met. 
    // Ideally this is set in socket handler, but keeping structure for now.
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-green-900 to-gray-900 flex items-center justify-center p-4">
        <div className="bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-2xl w-full border-2 border-green-600">
          <h1 className="text-5xl font-bold text-center mb-2 text-green-400">
            🎮 Advanced Reversi AI
          </h1>
          <p className="text-center text-gray-400 mb-8">
            Minimax + Alpha-Beta Pruning | Transposition Tables | Iterative Deepening
          </p>
          ...
          <div className="space-y-4 mb-8">
            <div>
              ...
              <select
                ...
                className="w-full bg-gray-700 text-white rounded-lg p-3 border-2 border-gray-600 focus:border-green-500 focus:outline-none"
              >
          ...
          {showHvHSubMenu && (
            <div className="grid grid-cols-2 gap-4 mb-4 p-4 bg-gray-700 rounded-lg border-2 border-blue-500">
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white rounded-xl p-4 transition-all transform hover:scale-105 flex flex-col items-center gap-2 border-2 border-green-500"
              >
                <Users size={30} />
                <span className="font-bold text-md">Local HvH</span>
                <span className="text-sm text-green-200">2 players on one device</span>
              </button>
              <button
                onClick={() => startGame('online')}
                disabled={!isOnlineReady}
                className={`text-white rounded-xl p-4 transition-all flex flex-col items-center gap-2 border-2 ${
                  isOnlineReady ? 'bg-blue-600 hover:bg-blue-700 border-blue-500' : 'bg-gray-600 text-gray-400 border-gray-500'
                }`}
              >
                <Network size={30} />
                <span className="font-bold text-md">Online HvH</span>
                <span className="text-sm">{isOnlineReady ? 'Play against a friend' : 'Connecting...'}</span>
              </button>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* HvH Button */}
            <button
              onClick={() => setShowHvHSubMenu(!showHvHSubMenu)}
              className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl p-6 transition-all transform hover:scale-105 flex flex-col items-center gap-3 border-2 border-blue-500"
            >
              <Users size={40} />
              <span className="font-bold text-lg">Human vs Human</span>
              <span className="text-sm text-blue-200">Local or Online</span>
            </button>

            {/* HvAI Button (renamed from Users) */}
            <button
              onClick={() => startGame('hvai')}
              className="bg-orange-600 hover:bg-orange-700 text-white rounded-xl p-6 transition-all transform hover:scale-105 flex flex-col items-center gap-3 border-2 border-orange-500"
            >
              <Cpu size={40} />
              <span className="font-bold text-lg">Human vs AI</span>
              <span className="text-sm text-orange-200">Play against the AI</span>
            </button>
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-700 text-white rounded-xl p-6 transition-all transform hover:scale-105 flex flex-col items-center gap-3 border-2 border-purple-500"
            >
              <Brain size={40} />
              <span className="font-bold text-lg">AI Solver</span>
              <span className="text-sm text-purple-200">Get best moves</span>
            </button>
          </div>
          ...
          <div className="mt-8 p-4 bg-gray-700 rounded-lg border border-gray-600">
            ...
            <ul className="text-sm text-gray-300 space-y-1">
              <li>✅ Zobrist Hashing & Transposition Tables</li>
              <li>✅ 10+ Advanced Heuristics</li>
              <li>✅ Phase-Based Strategy (Opening/Mid/End)</li>
              <li>✅ Killer Move Heuristic & Move Ordering</li>
              <li>✅ Real-time Performance Analytics</li>
              <li>✅ AI Hint System for Human Players</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  // 4. Editor Screen (Custom Board Editor)
  if (gameMode === 'editor') {
    return (
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-xl p-4 mb-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => { setGameState('menu'); setGameMode(''); }}
                className="bg-gray-700 text-white px-4 py-2 rounded"
              >
                ← Menu
              </button>
              <h1 className="text-2xl font-bold text-green-400">
                🧠 AI Solver — Enter Board & Analyze
              </h1>
  ...
  // 5. Main Game Screen (Used for HvH, HvAI, AiVAi, Online)
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-green-900 to-gray-900 p-4">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="bg-gray-800 rounded-xl p-4 mb-4 flex flex-wrap items-center justify-between border-2 border-green-600 gap-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => {
                if(gameMode === 'online') resetOnlineState();
                setGameState('menu');
                setBoard([]);
              }}
              className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition-all"
            ...
            <h1 className="text-2xl font-bold text-green-400">
              {gameMode === 'hvai' ? '👤 Human vs AI' : 
               gameMode === 'hvh' ? '👥 Local Human vs Human' :
               gameMode === 'online' ? `🌐 Online HvH: ${onlineGameId}` :
          ...
          <div className="flex gap-2 flex-wrap">
            {/* UPDATED: Allow hint in HvAI (P1 only) OR HvH (P1 or P2) */}
            {((gameMode === 'hvai' && currentPlayer === 1) || gameMode === 'hvh') && !gameOver && (
              <button
                onClick={getAIHint}
                disabled={thinking}
                className="bg-blue-500 hover:bg-blue-600 disabled:bg-gray-600 text-white px-4 py-2 rounded-lg transition-all flex items-center gap-2"
              >
                <Lightbulb size={18} />
                Get Hint
              </button>
            ...
            <button
              onClick={() => initializeGame(boardSize, gameMode)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2">
            <div className="bg-gray-800 rounded-xl p-6 border-2 border-green-600">
              {(thinking && gameMode !== 'hvh' && gameMode !== 'online') && (
                <div className="mb-4 bg-yellow-900 border border-yellow-600 text-yellow-200 p-3 rounded-lg flex items-center gap-3">
              ...
              {gameOver && (
                <div className="mb-4 bg-blue-900 border-2 border-blue-500 text-white p-4 rounded-lg text-center">
              ...
              {board.length > 0 && (
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div className={`p-4 rounded-lg border-2 ${currentPlayer === 1 ? 'bg-gray-700 border-yellow-400' : 'bg-gray-900 border-gray-700'}`}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-900 border-2 border-gray-700" />
                      <div>
                        <div className="text-white font-bold">Black (Player 1) 
                          {currentPlayer === 1 ? (gameMode === 'online' && onlinePlayerColor !== 1 ? '(Opponent Turn)' : '(Your Turn)') : ''}
                        </div>
                      </div>
                    </div>
                  </div>

                  <div className={`p-4 rounded-lg border-2 ${currentPlayer === 2 ? 'bg-gray-700 border-yellow-400' : 'bg-gray-900 border-gray-700'}`}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-white border-2 border-gray-300" />
                      <div>
                        <div className="text-white font-bold">White (Player 2) 
                          {currentPlayer === 2 ? (gameMode === 'online' && onlinePlayerColor !== 2 ? '(Opponent Turn)' : (gameMode === 'hvai' ? '(AI Turn)' : '(Your Turn)')) : ''}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            {/* Performance Stats (Only show for AI modes) */}
            ...
            {/* Move History */}
            <div className="bg-gray-800 rounded-xl p-6 border-2 border-blue-600 max-h-96 overflow-y-auto">
              ...
              <div className="space-y-2">
                {moveHistory.length > 0 ? (
                  moveHistory.slice().reverse().map((move, idx) => (
                    <div key={idx} className="bg-gray-700 p-2 rounded text-sm text-white flex items-center gap-2">
                      <div className={`w-4 h-4 rounded-full ${move.player === 1 ? 'bg-gray-900' : 'bg-white'}`} />
                      <span>
                        {move.message || `Move ${moveHistory.length - idx}: Row ${move.row}, Col ${move.col}`}
                      </span>
                    </div>
                  ))
                ) : (
